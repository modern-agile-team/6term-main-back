...
export function ApiUpdateReComment() {
  return applyDecorators(
    ApiOperation({
      summary: '대댓글을 수정하는 API',
      description: '대댓글을 수정하는 API',
    }),
    ApiResponse({
      status: 200,
      description: '대댓글의 내용을 성공적으로 수정한 경우',
      content: {
        JSON: {
          example: {
            id: '대댓글 고유 id',
            userId: '대댓글 작성한 유저 id',
            commentId: '대댓글을 작성한 댓글의 id',
    ...
    ApiResponse({
      status: 500,
      description: '대댓글 수정중 오류가 발생했습니다',
      content: {
        JSON: {
          example: {
            statusCode: 500,
            message: '대댓글 수정 중 오류가 발생했습니다.',
          },
        },
      },
    }),
